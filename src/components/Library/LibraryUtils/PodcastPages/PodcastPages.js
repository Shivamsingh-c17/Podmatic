...
export default function PodcastPages(props) {
	// STATE MANAGEMENT
	...
	let n = props.podcastNFTs.length;
	let imagesArray = [];
	let pageDivArray = [];
	let j = 0;
	...
	for (let i = n - 1; i >= 0; i--) {
		...
		imagesArray.push(
			<div className="podcast_card_component" key={i}>
				<Link to={`/podcast-info/${props.podcastNFTs[i].id}`}>
					<div className="library_cards_cover_div justify-content-center align-items-center">
						<img src={`https://podmatic.infura-ipfs.io/ipfs/${props.podcastNFTs[i].hashes.imgHash}`} alt="podcast cover" />
						<div className="text-break">
							<p>#{`${props.podcastNFTs[i].id}`}</p>
							<span>{`${props.podcastNFTs[i].podcastName}`}</span>
						</div>
					</div>
				</Link>
			</div>
