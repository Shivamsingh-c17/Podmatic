...
const AudioPlayer = (props) => {
	...
	const startStopPodcast = useCallback(
		(prevValue) => {
			if (prevValue) {
	...
	const togglePlayPause = () => {
		...
		startStopPodcast(prevValue);
	};

	useEffect(() => {
		startStopPodcast(props.isPlaying);
	}, [props.isPlaying, startStopPodcast]);
		...
		<div className="audioPlayer glass_effect">
			...
			<div className="row p-3 audioPlayer_row">
				<div className="col-lg-1 d-lg-flex d-none justify-content-left align-items-left m-0 p-0">
					<img src={props.podcastCoverURL} className="cover_image" alt="podcast-cover" />
				</div>
				...
				<div className="col-2 col-lg-1 col-md-2 d-sm-flex d-none flex-end align-items-end justify-content-end m-0 p-0">
					{/* Info and Summary Button */}
					<Tooltip podcastInfoAudio={true} labelText="" message="Podcast quality may be a little low due to compression" />
					{props.lyricsPresent ? <Tooltip lyricsPresent={props.lyricsPresent} labelText="" message="Summary for this podcast are present" /> : null}
				</div>
			</div>
		</div>
	);
