...
export default function CoverCard(props) {
	const previousOwners = props.podcast.sales.previousOwners;
	const previousPrices = props.podcast.sales.previousPrices;
	const previousSaleTimes = props.podcast.sales.previousSaleTimes;
			...
			<div className="cover_card glass_effect glass_effect_border">
				<div className="row cover_card_row justify-content-center mb-4">
					<div className="col-12 p-0">
						<img className="album_cover" src={props.podcastCoverURL} width="100%" alt="album cover" />
				...
				{/* SOCIAL'S LINKS */}
				<Socials socials={props.podcast.links} />
					...
					<div className="col-lg-12 col-md-12 col-12 m-0 p-0">
						<div className="row justify-content-center m-0 p-0">
							<div className="col-lg-12 col-md-12 col-sm-10 col-xs-10 col-12 m-0 p-0">
								{props.account && props.account === props.podcast.currentOwnerAddress ? (
									<span>
										...
										{props.podcast.currentOwnerAddress !== props.podcast.podcasterAddress ? (
											<Link to={"/chat"}>
								...
								) : props.account && props.podcast.onSale ? (
									<button
										className="btn btn-primary shadow-sm submit-btn"
										onClick={() => {
											props.purchasePodcast(
												props.podcast.id,
												props.podcast.price,
												props.podcast.podcasterAddress,
				...
				{!props.podcast.onSale &&
				props.account === props.podcast.podcasterAddress &&
				props.podcast.podcasterAddress === props.podcast.currentOwnerAddress ? (
					<span>
						<small className="mt-4">Item currently not on the market</small>
						<h4
							className={"sale_history put_up_on_sale"}
							onClick={() => {
								props.toggleOnSale(props.podcast.id);
							}}
						>
							Put up on Sale
						</h4>
				...
				<span className="created_on text-break mt-3">Created on: {convertTimestampToDate(props.podcast.createTime)}</span>
							...
							<div className="row sale_modal_header">
								<div className="col-12 d-flex sale_modal_subheader flex-column px-0">
									...
									<div className="sale_modal_subtitle modal_green_text d-flex justify-content-center align-items-center">
										<div className="mx-sm-5 mx-2">{props.podcast.currentOwnerAddress}</div>
									</div>
								</div>
							</div>
							{props.podcast.podcasterAddress === props.podcast.currentOwnerAddress && props.podcast.sales.previousOwners.length === 0 ? (
												...
												<div className="row mt-4" key={index}>
													<div className="col sale_history_subsection p-0">
														<div className="sale_modal_subtitle d-flex justify-content-center align-items-center">
															<div className="mx-5">
																{previousOwners.length - index - 1 === 0
																	? props.podcast.podcasterAddress
																	: previousOwners[previousOwners.length - index - 1]}
															</div>
														</div>
													</div>
													<div className="col sale_history_subsection p-0 transfer_money d-flex flex-cloumn flex-wrap justify-content-center">
														<div className="col-12 p-0 m-0 pt-2 mt-1 d-flex justify-content-center">
															...
															<span className="transaction_money mx-3 fw-bold pt-1">
																{parseFloat(
																	web3.utils.fromWei(previousPrices[previousPrices.length - index - 1].toString(), "Ether")
																).toFixed(3)}
														...
														<span className="sale_modal_info mt-1">
															{convertTimestampToDate(previousSaleTimes[previousSaleTimes.length - index - 1])}
														</span>
													</div>
													<div className="col sale_history_subsection p-0">
														<div className="sale_modal_subtitle d-flex justify-content-center align-items-center">
															<div className="mx-5">
																{index === 0
																	? props.podcast.currentOwnerAddress
																	: previousOwners[previousOwners.length - index]}
															</div>
														</div>
													</div>
												</div>
														...
														<div className="col sale_history_subsection p-0 transfer_money d-flex flex-cloumn flex-wrap justify-content-center">
															<div className="col-12 p-0 m-0 pt-2 mt-1 d-flex justify-content-center">
																...
																<span className="transaction_money mx-3 fw-bold pt-1">
																	{parseFloat(
																		web3.utils.fromWei(
																			previousPrices[previousPrices.length - index - 1].toString(),
																			"Ether"
																		)
																	).toFixed(3)}
																	<span className="transaction_money_unit">&nbsp;&nbsp;MATIC</span>
																...
																{icon_arrowUp}
																<span className="sale_modal_info mt-2 ml-3">
																	on {convertTimestampToDate(previousSaleTimes[previousSaleTimes.length - index - 1])}
																</span>
															</div>
														</div>
													</div>
							...
							<div className="row mt-5">
								<div className="col-12 mt-4 d-flex sale_modal_creation p-4 flex-column justify-content-center align-items-center">
									<h5 className="sale_modal_title">
										Creation <span className="created_on">({convertTimestampToDate(props.podcast.createTime)})</span>
									</h5>
									<div className="sale_modal_artist_address text-center text-break">(Artist Address: {props.podcast.podcasterAddress})</div>
								</div>
							</div>
						</div>
					</div>
					...
					<div className="modal-content sale_modal_content p-sm-5 p-4">
						...
						<form
							onSubmit={(event) => {
								...
								props.updatePrice(props.podcast.id, _newPrice);
							}}
						>
							<div className="modal-body">
								<div className="row">
