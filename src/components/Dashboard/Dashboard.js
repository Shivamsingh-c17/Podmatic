...
import dashboardSvg from "../../assets/new-assets/podcast-icon.svg";

export default function Dashboard(props) {
	function isOwner(_podcast) {
		return _podcast.currentOwnerAddress === props.account;
	}

	function isCreator(_podcast) {
		return _podcast.podcasterAddress === props.account;
	}
									...
									{props.account ? (
										<Fragment>
											...
											<Link to={"/chat"}>
												<button className="btn btn-primary shadow-sm chat_with_artist_btn mt-3">
													<span>
														Chat with the podcasters &nbsp;<i className="fas fa-comments"></i>
													</span>
												</button>
											</Link>
										</Fragment>
										...
										<div>
											...
											<span className="connect-metamask-message">
												...
												{`	`}
												You must connect to metamask to buy and sell NFTs on Podmatic
											</span>
										</div>
									)}
								</div>
						...
						<div className="owned-by-user glass_effect glass_effect_border">
							...
							<div className="container-fluid d-flex flex-wrap justify-content-center owned-nfts-div">
								<div className="row mt-3 p-0 text-center justify-content-center">
									{props.podcastNFTs.filter(isOwner).length > 0 ? (
										props.podcastNFTs.map((podcast, key) => {
											return (
												<Fragment key={key}>
													{podcast.currentOwnerAddress === props.account ? (
														<div className="podcast_card_component">
															<Link to={`/podcast-info/${podcast.id}`}>
																<div className="library_cards_cover_div">
																	<img
																		src={`https://podmatic.infura-ipfs.io/ipfs/${podcast.hashes.imgHash}`}
																		alt="podcast cover"
																	/>
																	<div className="text-break">
																		<p className="info_podcast_card_dashboard">#{`${podcast.id}`}</p>
																		<span className="info_podcast_card_dashboard">{`${podcast.podcastName}`}</span>
																	</div>
																</div>
															</Link>
														</div>
					...
					<br /> */}
						<div className="created-by-me glass_effect glass_effect_border">
							...
							<div className="container-fluid d-flex flex-wrap justify-content-center created-nfts-div">
								<div className="row mt-3 p-0 text-center justify-content-center">
									{props.podcastNFTs.filter(isCreator).length > 0 ? (
										props.podcastNFTs.map((podcast, key) => {
											return (
												<Fragment key={key}>
													{podcast.podcasterAddress === props.account ? (
														<div className="podcast_card_component">
															<Link to={`/podcast-info/${podcast.id}`}>
																<div className="library_cards_cover_div">
																	<img
